...
#endif
Argument::Argument(int argc, char **argv, std::vector<int> ind_val) {
	argc_m = argc;
	argv_m = argv;
	populationSize_m = 30;
	iterations_m = 500;
...
void Argument::Parse()
{
	...
	for (register int index = 1; index < argc_m; index++) {
		std::cout << argv_m[index] << std::endl;
		if (strcasecmp(argv_m[index], "-name") == 0) {
			...
			benchmark_m = BenchmarkFactory::Get()->CreateBenchmark(name, indices_validos);
