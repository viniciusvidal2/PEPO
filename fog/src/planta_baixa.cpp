...
int main(int argc, char **argv)
{
  ...
  n_.param<float >("lado_area", lado_area    , 20  ); // Formar um quadrado para ter uma planta baixa mais local
  ...
  pass.setFilterLimits(-lado_area/2, lado_area/2); // Quadrado em Z
  pass.setInputCloud(nuvem_inicial);
  pass.filter(*nuvem_inicial);
