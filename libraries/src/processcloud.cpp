...
void ProcessCloud::preprocess(PointCloud<PointT>::Ptr cin, PointCloud<PointTN>::Ptr out, float vs, float d, int fp,
  ...
  PointCloud<PointT>::Ptr depth_temp (new PointCloud<PointT>);
  for(size_t i=0; i<cin->size(); i++){
    if(sqrt( pow((*cin)[i].x, 2) + pow((*cin)[i].y, 2) + pow((*cin)[i].z, 2) ) <= d)
      depth_temp->push_back((*cin)[i]);
  }
  *cin = *depth_temp;
  ...
  if(!com_octree){
    ...
    this->filterCloudDepthCovariance(cin, 30, 1.5, depth_cov_filter);
    ...
    while(!so_pequenos){
//      cout << "\nTem alguma leaf grande aqui ainda na busca ...  " << cin_clusters.size() << endl;
      so_pequenos = true;
      // Se achar algum cluster grande ainda, separar indice e nuvem correspondente
      for(size_t i=0; i<cin_clusters.size(); i++){
//        cout << "\nO tamanho do cluster " << i+1 << " e " << cin_clusters[i].size() << endl;
        if(cin_clusters[i].size() > 30000){
          so_pequenos = false;
          indice_grande = i;
          *nuvem_grande = cin_clusters[i];
//          cout << "\nIndice e tamanho do maior cluster " << i+1 << " e " << cin_clusters[i].size() << endl;
//          savePLYFileASCII(pasta+"nuvem_grande_"+std::to_string(int(rand()%10))+".ply", *nuvem_grande);
          break;
        }
...
#pragma omp parallel for
    for(size_t i=0; i<out_clusters.size(); i++){
      ...
      *temp = cin_clusters[i];
      // Calcular filtro de covariancia na regiao mais proxima
//      float depth_cov_filter = 3;
//      this->filterCloudDepthCovariance(temp, 30, 1.5, depth_cov_filter);
      // Filtro de ruidos aleatorios
      StatisticalOutlierRemoval<PointT> sor;
      sor.setMeanK(30);
      sor.setStddevMulThresh(2);
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void ProcessCloud::filterRayCasting(PointCloud<PointT>::Ptr in, float lat_c, float lon_c, float lat_fov, float lon_fov){
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void ProcessCloud::cleanMisreadPoints(PointCloud<PointT>::Ptr cloud){
